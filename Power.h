...
float pmu_temperature = PMU_TEMP_MIN-1;
  ...
  const uint8_t pin_ctrl = PIN_T096_ADC_EN;
  float bat_p_samples[BAT_SAMPLES];
  float bat_v_samples[BAT_SAMPLES];
  uint8_t bat_samples_count = 0;
  int bat_discharging_samples = 0;
...
void measure_battery() {
    ...
    #elif BOARD_MODEL == BOARD_HELTEC_T096
      // Voltage divider ratio 4.916, 3.6v reference, 10-bit resolution
      float battery_measurement = (float)(analogRead(pin_vbat)) * 0.017300;
    #elif BOARD_MODEL == BOARD_TECHO
